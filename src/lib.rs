...
impl KMeans {
    ...
    pub fn train(&mut self, data: ArrayView2<f32>, num_threads: Option<usize>) -> Result<Vec<Vec<usize>>> {
        let (n, _) = data.dim();
    ...
    pub fn assign(&self, data: ArrayView2<f32>, k: usize) -> Result<Vec<Vec<usize>>> {
        ...
        if k == 0 {
            return Err(KMeansError::InvalidParameter("k must be positive".to_string()));
        }
...
impl KMeans {
    ...
    fn assign_clusters(&mut self, data: &ArrayView2<f32>, data_norms: Option<&Array1<f32>>) {
        ...
        if self.euclidean {
            ...
            let assignments: Vec<usize> = iter.map(|i| {
                let point = data.row(i);
                let dots = centroids.dot(&point);
    ...
    fn update_centroids(&mut self, data: &ArrayView2<f32>) {
        ...
        for j in 0..self.n_clusters {
            if self.cluster_sizes[j] > 0.0 {
                centroids.row_mut(j).mapv_inplace(|x| x / self.cluster_sizes[j]);
            }
        }
    ...
    fn split_clusters(&mut self, data: &ArrayView2<f32>) {
        ...
        for i in 0..self.n_clusters {
            if self.cluster_sizes[i] == 0.0 {
                ...
                let mut j = 0;
                loop {
                    let p = (self.cluster_sizes[j] - 1.0) / (data.nrows() as f32 - self.n_clusters as f32);
    ...
    fn balance_clusters(&mut self, data: &ArrayView2<f32>, data_norms: Option<&Array1<f32>>) -> Result<()> {
        let centroids = self.centroids.as_ref().unwrap();
        ...
        for (i, point) in data.outer_iter().enumerate() {
            let cluster = self.assignments[i];
            unnormalized_centroids.row_mut(cluster).scaled_add(1.0, &point);
        }
        
        let mut n_min = self.cluster_sizes.iter().cloned().fold(f32::INFINITY, f32::min);
        let mut n_max = self.cluster_sizes.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
        ...
        while n_max - n_min > 0.5 + self.max_balance_diff as f32 {
            for i in 0..data.nrows() {
                ...
                unnormalized_centroids.row_mut(old_cluster).scaled_add(-1.0, &point);
                ...
                if n_old > 0.0 {
                    ...
                    if let Some(ref mut centroids) = self.centroids {
                        ...
                        if !self.euclidean {
                            let norm = (centroids.row(old_cluster).dot(&centroids.row(old_cluster))).sqrt();
                            if norm > 0.0 {
                ...
                self.cluster_sizes[old_cluster] = self.cluster_sizes[old_cluster] - 1.0 + PARTLY_REMAINING_FACTOR;
                ...
                for p in 0..self.n_clusters {
                    if self.cluster_sizes[old_cluster] > self.cluster_sizes[p] && penalties_2[p] != 0.0 {
                ...
                unnormalized_centroids.row_mut(min_cluster).scaled_add(1.0, &point);
                
                // Update new centroid
                let new_centroid = &unnormalized_centroids.row(min_cluster) / self.cluster_sizes[min_cluster];
                if let Some(ref mut centroids) = self.centroids {
                    centroids.row_mut(min_cluster).assign(&new_centroid);
                    if !self.euclidean {
                        let norm = (centroids.row(min_cluster).dot(&centroids.row(min_cluster))).sqrt();
                        if norm > 0.0 {
            ...
            n_min = self.cluster_sizes.iter().cloned().fold(f32::INFINITY, f32::min);
            n_max = self.cluster_sizes.iter().cloned().fold(f32::NEG_INFINITY, f32::max);
            ...
            if self.verbose {
                let cost = self.compute_cost(data);
                println!("Balance iteration {} | Cost: {:.6} | Max diff: {:.2}", 
                        iters, cost, n_max - n_min);
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_assign() {
        let train_data = Array2::from_shape_vec((6, 2), vec![
            1.0, 1.0,
            1.1, 1.1,
            5.0, 5.0,
            5.1, 5.1,
        ...
        let test_data = Array2::from_shape_vec((2, 2), vec![
            1.05, 1.05,
            9.05, 9.05,
        ]).unwrap();
